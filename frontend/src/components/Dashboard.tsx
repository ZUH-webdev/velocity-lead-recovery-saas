...
import type { ActivityEvent, CalendarStatus, Lead, RecoveryMetrics } from '../types';
...
const Dashboard = ({ onNavigate = () => {}, searchTerm: externalSearchTerm, onSearchChange }: Props) => {
  ...
  const updateSearchTerm: (value: string) => void = onSearchChange || setLocalSearchTerm;
  ...
  const filteredLeads = leads.filter((lead) => {
    ...
    return (
      ...
      lead.source?.toLowerCase().includes(searchValue) || false
    );
  });

  const dashboardMetrics = (metrics as Partial<RecoveryMetrics>) || generateMockRecoveryMetrics();
  ...
  return (
    ...
    >
      ...
      <motion.section variants={itemVariants}>
        <PremiumFunnelChart metrics={dashboardMetrics} />
      </motion.section>
