import type { ActivityEvent, CalendarEvent, CalendarStatus, ConversationMessage, Lead, RecoveryMetrics } from '../types';

// Mock data generator for development
export const generateMockLeads = (count = 12): Lead[] => {
...
export const generateMockRecoveryMetrics = (): RecoveryMetrics => ({
...
export const generateMockActivities = (count = 5): ActivityEvent[] => {
  ...
  return Array.from({ length: count }, (_, i): ActivityEvent => ({
    id: `activity-${i + 1}`,
...
export const generateMockConversation = (): ConversationMessage[] => [
  {
    id: 'msg-1',
    sender: 'ai',
...
export const generateMockCalendarStatus = (): CalendarStatus => ({
  isConnected: true,
...
export const generateMockEvents = (count = 8): CalendarEvent[] => {
  const eventTypes = ['Checkup', 'Cleaning', 'Consultation', 'Follow-up', 'Root Canal', 'Crown'];
  const today = new Date();

  return Array.from({ length: count }, (_, i): CalendarEvent => {
